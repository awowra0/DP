...
class LibraryCatalog(object):
    ...
    It uses Singleton pattern design (one catalog is enough) and Iterator (to show next books in catalog).
...
class DataAdapter:
    ...
    Methods:
    read (catalog: LibraryCatalog, filename: str) -> int: Attempts to recognise file type and send it to proper reader fuction. Returns 1/0 if it is xml and made no/any mistake, returns 6/5 if it is csv and made no/any mistake, returns 11/10 if it is json and made no/any mistake or return -1 if file type is unknown.
    ...
    read_json (catalog: LibraryCatalog, filename: str) -> int: Reads json file and tries to add found books to catalog. Returns 11 if no mistake made or 10 otherwise.
